...
5. 리소스 매니저의 submitApplication()을 호출하여 잡을 제출함
...
**스트리밍**
![image](https://user-images.githubusercontent.com/28076434/56328640-d95be380-61ba-11e9-9f29-def0902ea07c.png)
...
(예: 맵 태스크는 처리할 파일명을, 맵 또는 리듀스 태스크는 태스크 시도 횟수를 알 수 있음)
<br>
![image](https://user-images.githubusercontent.com/28076434/56297296-0f717700-616b-11e9-8f15-4e686fd4dc43.png)
![image](https://user-images.githubusercontent.com/28076434/56297322-17c9b200-616b-11e9-8c90-f3f2d99b2f10.png)

* 위 표의 속성은 이전 맵리듀스 API가 제공하는 Mapper 또는 Reducer의 configure() 메서드 구현을 통해 얻을 수 있는 잡의 환경 설정으로 접근 가능함
* 새로운 API에서 이 속성은 Mapper 또는 Reducer의 모든 메서드에 전달된 콘텍스트 객체로부터 접근 가능함

**스트리밍 환경변수**
* 하둡은 잡 환경 설정 파라미터를 스트리밍 프로그램의 환경변수로 설정함
* 알파벳이나 숫자 이외의 문자는 밑줄로 대체하여 유효한 이름으로 생성함
* 파이썬 스트리밍 스크립트에서 mapreduce.job.id 속성 값을 얻어오는 방법
```python
os.environ["mapreduce_job_id"]
```
* 스트리밍 프로세스 실행 시 -cmdenv 옵ㅇ션으로 환경변수 설정이 가능함
```python
...
=> 잡 수행을 지연 시키는 태스크에 대한 고민이 필요
**투기적 실행** 
* 예상했던 것보다 태스크 수행이 느릴 경우를 감지하여 다른 동일한 예비 태스크를 실행하는 것
* 모든 태스크의 진행 상황을 기록하고 심각하게 느리게 수행 중인 태스크에 대해 투기적 복제 태스크를 실행함
* 이들 중 하나의 태스크가 성공적으로 완료되면 실행 중인 중복 태스크는 강제 종료 됨
* 투기적 실행은 태스크를 멈추거나 느리게 하는 버그가 존재할 경우, 문제를 회피하기 위한 것이지 안정적으로 잡을 실행하는 것은 아님. 따라서 버그를 고쳐야 함.

**투기적 실행 속성**
![image](https://user-images.githubusercontent.com/28076434/56299270-a986ee80-616e-11e9-8d36-315e3a7d0024.png)
투기적 실행을 위 표의 속성을 이용하여 활성화, 비활성화 가능함
* 투기적 실행은 클러스터 효율성 측면에서 비용이 발생함
=> 혼잡한 클러스터에서 개별 잡의 실행 시간을 줄이기 위해 투기적 실행으로 중복 태스크가 실행되면 전반적인 단위 시간당 산출물은 줄어들게 됨
=> 이런 이유로 클러스터 관리자는 클러스터 상에서 투기적 실행을 끄는 것을 선호
